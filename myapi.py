...
class ModelManager:
    ...
    def query_text_openai(self, model, user_prompt, system_prompt, image=None):
    ...
    def text_query(self, user_prompt, system_prompt, image=None):
        if self.config['Text-to-Text']['source'] == 'openai':
            return self.query_text_openai(self.config['Text-to-Text']['model'], user_prompt, system_prompt, image)
    ...
    def image_query(self, prompt, save_path, img_format):
        if self.config['Text-to-Image']['source'] == 'openai':
            model = self.config['Text-to-Image']['model']
            img = self.query_image_openai(prompt, model)
            self.save_image_from_url(img, save_path, img_format)
            return True
